...
check('toolsToPayload 默认总数 = 19 (16实+3元, 全部启用)', payload.length === 19, 'actual=' + payload.length)
const payloadNoExec = toolsToPayload(agentTools, { exec: false })
check('toolsToPayload(exec=false) 总数 = 18 (不含exec)', payloadNoExec.length === 18, 'actual=' + payloadNoExec.length)
const payloadNoExecNoSearch = toolsToPayload(agentTools, { exec: false, web_search: false })
check('toolsToPayload(exec+web_search=false) 总数 = 17', payloadNoExecNoSearch.length === 17, 'actual=' + payloadNoExecNoSearch.length)
...
check('默认 payload 含 exec', names.includes('exec'))
check('默认 payload 含 web_search', names.includes('web_search'))
const namesNoExec = payloadNoExec.map((p) => (p as any).function.name as string)
check('exec=false 不含 exec', !namesNoExec.includes('exec'))
check('exec=false 含 web_search', namesNoExec.includes('web_search'))
const namesNoExecNoSearch = payloadNoExecNoSearch.map((p) => (p as any).function.name as string)
check('exec+web_search=false 不含 web_search', !namesNoExecNoSearch.includes('web_search'))
...
check('findTool(exec, {exec:false})=undefined', findTool('exec', { exec: false }) === undefined)
check('findTool(exec, {})=exec', findTool('exec', {}) === ex)
...
check('findTool(web_search, {web_search:false})=undefined', findTool('web_search', { web_search: false }) === undefined)
check('findTool(web_search, {})=web_search', findTool('web_search', {}) === wsTool)
...
const sp1 = buildSystemPrompt('base', true, true, 0)
check('planMode=true 含 create_plan', sp1.includes('create_plan'))
check('planMode=true 含等待用户确认', sp1.includes('等待用户确认'))
check('planMode=true 含严禁直接调用', sp1.includes('严禁'))
const sp1b = buildSystemPrompt('base', true, false, 0)
check('planMode=false 不含等待用户确认', !sp1b.includes('等待用户确认'))
check('planMode=false 含简单问题可直接回答', sp1b.includes('简单问题可直接回答'))
const sp2 = buildSystemPrompt('base', false, true, 0)
